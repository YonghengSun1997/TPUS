"""Prepare a combined MultiTalent training from multiple nnU-Net datasets.

TPUS release modification: removed a dataset-converter-only import and fixed
the existing-plan lookup so this public entry point does not replan datasets
whose ``nnUNetPlans.json`` already exists.
"""
...
def prepare_MT_training():
    ...
    parser.add_argument('--omit_preprocessing', action='store_true', required=False,
                        help='[OPTIONAL] write plans without running preprocessing')
    ...
    if args.batch_size is not None:
        for config in target_plan_MT['configurations'].keys():
    ...
    for id in args.d:
        ...
        default_plan_file = join(nnUNet_preprocessed, d_name, 'nnUNetPlans.json')
        if not isfile(default_plan_file):
            ...
            default_plan = load_json(default_plan_file)

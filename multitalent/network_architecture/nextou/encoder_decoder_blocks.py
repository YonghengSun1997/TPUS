# TPUS modification: adapts NexToU Pool/Swin GNN blocks to MultiTalent architecture builders.
import numpy as np
import torch
...
from multitalent.network_architecture.nextou.torch_nn import BasicConv, batched_index_select, act_layer
from multitalent.network_architecture.nextou.torch_edge import DenseDilatedKnnGraph
from multitalent.network_architecture.nextou.pos_embed import get_2d_relative_pos_embed, get_3d_relative_pos_embed
...
class SwinGNNBlocks(nn.Module):
    def __init__(self, channels, img_shape, index, opt=None, conv_op=nn.Conv3d, norm_op=nn.BatchNorm3d, norm_op_kwargs=None,
        ...
        if conv_op == nn.Conv2d:
            ...
            max_num = max(1, int(H_min * W_min // 2))
            k_candidate_list = [2, 4, 8, 16, 32]
            max_k = max(1, min(k_candidate_list, key=lambda x: abs(x - max_num)))
            min_k = max(1, max_num // (2 * 2))
            ...
            k_list = [max(1, k) for k in k_list]
            max_dilation = max(1, (H_min * W_min) // max(k_list))
            window_size = img_min_shape
            window_size_n = window_size[0] * window_size[1]   
        elif conv_op == nn.Conv3d:
            H_min, W_min, D_min = img_min_shape
            max_num = max(1, int(H_min * W_min * D_min // 3))
            k_candidate_list = [2, 4, 8, 16, 32]
            max_k = max(1, min(k_candidate_list, key=lambda x: abs(x - max_num)))
            min_k = max(1, max_num // (2 * 2 * 2))
            if pool_op_kernel_sizes_len >= 5:
                k_list = [min(min_k, max_k), min(min_k*2, max_k), min(min_k*2, max_k), min(min_k*4, max_k), min(min_k*8, max_k)] + [min(min_k*16, max_k)] * (pool_op_kernel_sizes_len - 5)
            else:
                k_list = [min(min_k, max_k), min(min_k*2, max_k), min(min_k*2, max_k), min(min_k*4, max_k), min(min_k*8, max_k)][0:pool_op_kernel_sizes_len]
            ...
            k_list = [max(1, k) for k in k_list]
            max_dilation = max(1, (H_min * W_min * D_min) // max(k_list))
            window_size = img_min_shape

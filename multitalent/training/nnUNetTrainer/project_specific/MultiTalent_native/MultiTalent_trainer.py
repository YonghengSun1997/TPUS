...
# TPUS release modification: removed an unrelated Dataset201 case-specific sampling exception.
import os
import shutil
import sys
import subprocess
...
def format_module_tree_without_duplicates(module: nn.Module) -> str:
    """Format a complete module tree while expanding shared modules only once."""
    lines = []
    seen_modules = {}

    def render(current: nn.Module, path: str, label: str, depth: int) -> None:
        seen_modules[id(current)] = path

        extra = " ".join(current.extra_repr().split())
        description = current.__class__.__name__
        if extra:
            description += f"({extra})"

        direct_parameters = tuple(current.parameters(recurse=False))
        parameter_suffix = ""
        if direct_parameters:
            total = sum(parameter.numel() for parameter in direct_parameters)
            trainable = sum(parameter.numel() for parameter in direct_parameters if parameter.requires_grad)
            parameter_suffix = f" [params={total:,}, trainable={trainable:,}]"

        lines.append(f"{'  ' * depth}{label}: {description}{parameter_suffix}")

        for child_name, child in current._modules.items():
            if child is None:
                continue
            child_path = f"{path}.{child_name}" if path else child_name
            previous_path = seen_modules.get(id(child))
            if previous_path is not None:
                lines.append(f"{'  ' * (depth + 1)}{child_name}: <shared module; see {previous_path}>")
            else:
                render(child, child_path, child_name, depth + 1)

    render(module, "<root>", "<root>", 0)
    unique_parameters = tuple(module.named_parameters(remove_duplicate=True))
    total = sum(parameter.numel() for _, parameter in unique_parameters)
    trainable = sum(parameter.numel() for _, parameter in unique_parameters if parameter.requires_grad)
    lines.extend(("", f"Unique parameters: total={total:,}, trainable={trainable:,}"))
    return "\n".join(lines)
